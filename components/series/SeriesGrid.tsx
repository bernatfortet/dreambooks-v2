...
export function SeriesGridItem({ seriesId, slug, name, coverUrl }: SeriesGridItemProps) {
  return (
    <div className='group block'>
      <div className='aspect-32/25 relative bg-muted rounded-lg overflow-hidden mb-2'>
        <Link href={`/series/${slug}`} className='block h-full w-full'>
          {coverUrl ? (
            <Image
              src={coverUrl}
              alt={name}
        ...
        <SeriesProfileActions seriesId={seriesId} layout='card-overlay' />
      </div>
